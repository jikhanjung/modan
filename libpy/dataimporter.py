...
class ModanDataImporter:
  ...
  def openExcelFile(self, filepath):
    ...
    if len( sheet_list ) == 1 or \
       len( sheet_list ) == 3 and ( sheet_list[1].nrows == 0 ) and ( sheet_list[2].nrows == 0 ):
  ...
  def importMorphologikaFile(self,dataset):
    ...
    if len( objects ) != self.object_count:
      print 'number of objects does not match!! Supposed to be %d, but only %d found' %  ( self.object_count, len( objects ) )
    #print raw_object_list
    if len( objects[len(objects)-1].landmarks ) != self.landmark_count:
      print 'number of landmarks does not match for last object!! Supposed to be %d, but only %d found' % ( self.landmark_count, len( objects[len(objects)-1] ) )
    ...
    for i in range( len( group_info ) ):
      ...
      if sum > self.object_count:
        ...
        print "Group info " + ", ".join( temp_name ) + " (total " + str( sum ) + ") does not match with the number of objects " + str( self.object_count ) + ". No more group info will be processed."
        break
