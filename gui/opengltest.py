...
class MdCanvasBase( glcanvas.GLCanvas ):
  ...
  def IsCursorOnLandmark(self,x,y):
    ...
    glFrustum( self.frustum_args['width'] * -1.0, self.frustum_args['width'], self.frustum_args['height'] * -1.0, self.frustum_args['height'], self.frustum_args['znear'], self.frustum_args['zfar'] )
  ...
  def OnDraw(self):
    ...
    if self.dataset != None and (self.dataset.object_list )>0:
      ...
      for mo in self.dataset.object_list:
        ...
        self.DrawObject( mo, xangle, yangle, self.lm_radius, color=l_color, show_index = False, single_object_mode = somode )
